  ...
  if(preg_match("#^[a-f0-9]{16}$#", strtolower($_GET['key'])) {
    ...
    if(isset($_GET['e']) && !empty($_GET['e'])) {
      ...
      if(isset($_GET['a']) && !empty($_GET['a'])) {
        ...
        $requete_email = mysql_query("SELECT count(*) FROM tc_follow WHERE email = '".$_GET['e']."' AND article = '".$_GET['a']."'");
        
        if ($requete_email) {
          $requete_key = mysql_query("SELECT key FROM tc_keys WHERE email = '".$_GET['e']."'");
